		...
		implements ActionListener, VetoableChangeListener
{
	...
	public void setFrom(final I_AD_User newFrom)
	{
		m_from = newFrom;
		if (newFrom == null
				|| !Services.get(IUserBL.class).isEMailValid(newFrom)
				|| !Services.get(IUserBL.class).isCanSendEMailFromUser(newFrom))
		{
	...
	public void vetoableChange(final PropertyChangeEvent evt) throws PropertyVetoException
	{
		...
		if (source.equals(fUser))
		{
			...
			if (value instanceof Integer)
			{
				...
				m_user = Services.get(IUserDAO.class).retrieveUser(Env.getCtx(), AD_User_ID);
		...
		else
		{
			...
			{
				final int AD_User_ID = ((Integer)value).intValue();
				m_ccuser = Services.get(IUserDAO.class).retrieveUser(Env.getCtx(), AD_User_ID);
