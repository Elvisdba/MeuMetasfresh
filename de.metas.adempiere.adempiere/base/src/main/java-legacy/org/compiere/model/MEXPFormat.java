...
public class MEXPFormat extends X_EXP_Format
{
			...
			final Properties ctx, final String value, final int AD_Client_ID, final String version, final String trxName)
	{
		...
		final StringBuilder whereClause = new StringBuilder(X_EXP_Format.COLUMNNAME_Value).append("=?")
				.append(" AND AD_Client_ID IN (?, 0)")
				.append(" AND ").append(X_EXP_Format.COLUMNNAME_Version).append(" = ?");

		final MEXPFormat retValue = new Query(ctx, X_EXP_Format.Table_Name, whereClause.toString(), trxName)
