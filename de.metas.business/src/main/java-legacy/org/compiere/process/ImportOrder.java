		...
		try
		{
			...
			while (rs.next ())
			{
				...
				for (final I_C_BPartner_Location currentLoc : Services.get(IBPartnerDAO.class).retrieveBPartnerLocations(bp))
				{
					...
					{
						if (locationBL.equals(currentLoc.getC_Location(), imp.getC_Country_ID(), imp.getC_Region_ID(), 
				...
				if (imp.getContactName () != null 
					...
					|| imp.getPhone () != null)
				{
					List<I_AD_User> users = Services.get(IBPartnerDAO.class).retrieveContacts(bp);
					I_AD_User user = null;
