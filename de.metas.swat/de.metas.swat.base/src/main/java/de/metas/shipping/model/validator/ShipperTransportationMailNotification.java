...
public class ShipperTransportationMailNotification implements ModelValidator
{
		...
		if (po instanceof MMShipperTransportation && type == TIMING_AFTER_COMPLETE)
		{
			MMShipperTransportation ship = (MMShipperTransportation)po;
			{
				...
				for (MMShippingPackage sp : getShippingPackage(ship.getM_ShipperTransportation_ID()))
				{
					...
					I_C_Order order = sp.getM_InOut().getC_Order();
					if (order != null)
					...
					if (user == null)
						...
						orderUser = user;
					//
					I_C_BPartner partner = user.getC_BPartner();
					if (partner.isShippingNotificationEmail() && user.isDefaultContact())
					{
						String message = sendEMail(text, sp.getM_InOut(), orderUser);
	...
	private String sendEMail(final MADBoilerPlate text, final I_M_InOut io, final I_AD_User orderUser)
	{
		...
		MADBoilerPlate.sendEMail(new IEMailEditor()
		{
			...
			{
				return InterfaceWrapperHelper.getModelTableId(io);
			}

			@Override
			public int getRecord_ID()
			{
				return io.getM_InOut_ID();
			}
